...
#include "nav2_behavior_tree/zmq.hpp"
#include <future>
...
PublisherZMQ::PublisherZMQ(const BT::Tree &tree, unsigned max_msg_per_second,
    ...
    : BT::StatusChangeLogger(tree.rootNode()), tree_(tree),
      min_time_between_msgs_(std::chrono::microseconds(1000 * 1000) /
                             max_msg_per_second),
      send_pending_(false), zmq_(new Pimpl()) {
  // bool expected = false;
  ...
  thread_ = std::thread([this]() {
    while (active_server_) {
      ...
      try {
        ...
        std::cout << "[PublisherZMQ] just died. Exeption " << err.what()
                  << std::endl;
...
void PublisherZMQ::createStatusBuffer() {
  ...
  applyRecursiveVisitor(tree_.rootNode(), [this](BT::TreeNode *node) {
...
void PublisherZMQ::callback(BT::Duration timestamp, const BT::TreeNode &node,
                            BT::NodeStatus prev_status, BT::NodeStatus status) {
  using namespace std::chrono;

  BT::SerializedTransition transition =
      BT::SerializeTransition(node.UID(), timestamp, prev_status, status);
  {
...
void PublisherZMQ::flush() {
  zmq::message_t message;
  {
    ...
    const size_t msg_size =
        status_buffer_.size() + 8 + (transition_buffer_.size() * 12);

    message.rebuild(msg_size);
    uint8_t *data_ptr = static_cast<uint8_t *>(message.data());

    // first 4 bytes are the side of the header
    flatbuffers::WriteScalar<uint32_t>(
        data_ptr, static_cast<uint32_t>(status_buffer_.size()));
    ...
    flatbuffers::WriteScalar<uint32_t>(
        data_ptr, static_cast<uint32_t>(transition_buffer_.size()));
    data_ptr += sizeof(uint32_t);
