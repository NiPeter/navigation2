...
WaitForNewGoal::WaitForNewGoal(const std::string &xml_tag_name,
                               const BT::NodeConfiguration &conf)
    : ActionNodeBase(xml_tag_name, conf) {}

BT::NodeStatus WaitForNewGoal::tick() {
...
BT_REGISTER_NODES(factory) {
  BT::NodeBuilder builder = [](const std::string &name,
    ...
    return std::make_unique<nav2_behavior_tree::WaitAction>(name, "wait",
                                                            config);
  };

  factory.registerBuilder<nav2_behavior_tree::WaitAction>("Wait", builder);

  factory.registerNodeType<nav2_behavior_tree::WaitForNewGoal>(
      "WaitForNewGoal");
}
